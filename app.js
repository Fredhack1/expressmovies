...
app.use(expressJwt.expressjwt({ secret: secret, algorithms: ['HS256'] }).unless({ path: ['/', '/movies', '/movie-search', '/login'] }));
...
const fakeUser = { email: "testuser@gmail.com", password: "qsd" };
...
app.post('/login', urlencodedParser, (req, res) => {
    ...
    if (!req.body) {
        ...
        if (fakeUser.email === req.body.email && fakeUser.password === req.body.password) {
            const myToken = jwt.sign({ iss: 'https://expressmovies.cm', user: 'Samy', role: 'moderator' }, secret);
            res.json(myToken);
